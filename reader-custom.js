...
var conf_file = './config/config-custom.json';
...
if ((address.length > 2 && message) && ((trimMessage.match(/E\d{11}/g)) || (trimMessage.match(/N\d{11}/g)) || (trimMessage.match(/J\d{11}/g)) || (trimMessage.match('Part 2 of 2')) || (trimMessage.match('Branch for cover')))) {    
    var padAddress = padDigits(address,7);
    console.log(colors.red(time+': ')+colors.yellow(frequency+': ')+colors.blue(padAddress+': ')+colors.success(trimMessage));
    ...
    setTimeout(sendPage, 0, form, 0);
  } else {
    console.log(colors.red(time+': ')+colors.yellow(frequency+': ')+colors.grey(line));
  }
